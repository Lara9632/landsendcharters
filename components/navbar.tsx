        ...
        <div
          ...
          <NavbarContent className="hidden sm:flex ml-2" style={{ justifyContent: 'space-between', gap: '2rem' }}>
            <NavbarItem className="gap-2 items-center text-xl">
              <NextLink className="flex items-center gap-2 text-[#111D56]" href="tel:8002815778">
                LANDS END CHARTERS 1-800-281-5778
              </NextLink>
            </NavbarItem>
              ...
              {siteConfig.navItems.map(item =>
                item?.subItems ? (
                  <Dropdown
                    ...
                    classNames={{
                      base: '',
                      content: 'text-[#111D56] text-[22.5px] font-bold uppercase gap-0 min-w-0',
                      trigger: '',
                      backdrop: ''
                    }}>
                    <NavbarItem>
                    ...
                    <DropdownMenu
                      ...
                      itemClasses={{
                        ...
                        title: 'text-[#111D56] text-[22.5px] font-semibold uppercase gap-0 min-w-0 hover:text-white '
                      }}
                      aria-label="Dynamic Actions"
                      items={siteConfig.navItems}>
                      {item.subItems.map(subItem => (
                        <DropdownItem key={subItem.label} style={{ fontSize: '22.5px' }}>
                          {subItem.label}
                        </DropdownItem>
                      ))}
                    </DropdownMenu>
          ...
          <div className="mx-4 mt-2 flex flex-col gap-2">
            {siteConfig.navMenuItems.map((item, index) => (
              <NavbarMenuItem key={`${item}-${index}`}>
                <Link color={index === 0 ? 'primary' : 'foreground'} href="#" size="lg">
                  {item.label}
                </Link>
              </NavbarMenuItem>
            ))}
